...
if(!is.na(cur.jobnum)) {
  system(sprintf("sbatch --dependency=afternotok:%d ingest.r",cur.jobnum),intern=TRUE)
}

srcdir <- 'from_weir/portal.nccs.nasa.gov/datashare/gmao/geos_carb/MiCASA/v1/netcdf/MICASA_D_FLUX/'
...
month <- 1
day <- 1
regs <- load.ncdf(sprintf('%s/%d/%02d/MiCASAv1_flux.x3600_y1800.daily.%d%02d%02d.nc',srcdir,year,month,year,month,day))
...
for (year in 2001:2023) {
  ...
  for (month in 1:12) {
    ...
    for (day in 1:dpm[month]) {
      ...
      srcnm <- sprintf('%s/%d/%02d/MiCASAv1_flux.x3600_y1800.daily.%d%02d%02d.nc',srcdir,year,month,year,month,day)
      ncout <- sprintf('%s/MiCASAv1_flux.x360_y180.daily.%d%02d%02d.nc',outdir,year,month,day)
