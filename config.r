...
micasa.config <- function() {
  list(
    ...
    mail.user    = (function(v) if (nzchar(v)) v else stop("MAIL_USER must be set"))(Sys.getenv("MAIL_USER", "")),
    base.dir     = (function(v) if (nzchar(v)) v else stop("BASE_DIR must be set"))(Sys.getenv("BASE_DIR", "")),
