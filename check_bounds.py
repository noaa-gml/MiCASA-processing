...
a precise carbon budget. The unit conversion is `flux_to_tgc_per_year`,
a pure (numpy-only) function unit-tested by tests/test_check_bounds.py;
`xarray` is imported lazily inside main() so the test needs only numpy.
"""
import os
import sys
...
def flux_to_tgc_per_year(mean_flux):
    """Scale a global-mean carbon flux (gC m-2 s-1) to TgC per year.

    Multiplies by the whole Earth's surface area and one year of seconds,
    then converts grams to teragrams (1e15 g = 1 Tg... expressed as the
    /1e15 below, since the area carries the m-2). Deliberately crude --
    see the module docstring.
    """
    return mean_flux * EARTH_AREA * SECONDS_IN_YEAR / 1e15
...
def main():
    ...
    for tracer in ("NPP", "Rh", "FIRE", "FUEL"):
        ...
        print(f"  {tracer}: {flux_to_tgc_per_year(gmean):.6g} TgC / year")
    ds.close()
